...
class UpLoadImgView(LoginRequiredMixin,View):
    '''用户修改头像'''
    def post(self,request):
        img_form = UpLoadImgForm(request.POST,request.FILES,instance=request.user)
...
class UpdateEmailView(LoginRequiredMixin,View):
    ...
    def post(self,request):
        ...
        existd_records = EmailVerifyRecord.objects.filter(email=email,code=code,send_type='update_email')
        if existd_records:
            user = request.user
            user.email = email
            user.save()
